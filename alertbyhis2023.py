...
def filtph(filtday, todate, periodhigh):
    ...
    periodhigh.drop(periodhigh[periodhigh["filter"] == False].index, inplace=True)
...
def finAlert(path, stock_no, strtodate, stryesdate):
    ...
    try:
        findata = pd.read_csv(
            path + "/112newfinancing/" + stock_no + "stock_financing.csv", index_col=0
        )
        findelta = findata.at[strtodate, "融資前日餘額"] - findata.at[strtodate, "融資今日餘額"]
        perc = findelta / findata.at[strtodate, "融資今日餘額"]
        if perc > 0.15:
            Finmessage2 = "financing sold=", findelta, "(", round(perc, 2) * 100, "%)"
    except:
        findata = pd.read_csv(
            path + "/112newfinancing/" + stock_no + "stock_financing.csv", index_col=0
        )
        findelta = findata.at[stryesdate, "融資前日餘額"] - findata.at[stryesdate, "融資今日餘額"]
...
def alert(stock_no, stock_name, periodhigh, todayprice, yesprice, periodlow):
    ...
    for k in range(0, len(periodhigh)):
        ...
        if todayprice <= (periodhigh.at[k, "lhprice"] * 1.00) and yesprice > (
            periodhigh.at[k, "lhprice"] * 1.01
        ):
    ...
    for k in range(0, len(periodlow)):
        try:
            if todayprice <= (periodlow.at[k, "plprice"] * 1.00) and yesprice > (
                periodlow.at[k, "plprice"] * 1.01
            ):
...
def periodpeak(path, filename):
    ...
    periodhigh = pd.read_csv(
        path + "/periodhigh/" + filename + "periodhigh.csv", thousands=","
    )  # thounsands可以去千位符號
    periodlow = pd.read_csv(path + "/periodlow/" + filename + "periodlow.csv")
    # 只查看最近N天內的指標
    filtday = 60
...
def lowshadow(pf, twstrdate, stockname):
    ...
    try:
        if pf.at[twstrdate, "開盤價"] > pf.at[twstrdate, "收盤價"]:
            loshadow = (pf.at[twstrdate, "收盤價"] - pf.at[twstrdate, "最低價"]) / pf.at[
                twstrdate, "開盤價"
            ]
            # print('綠下影線=', loshadow)
        else:
            loshadow = (pf.at[twstrdate, "開盤價"] - pf.at[twstrdate, "最低價"]) / pf.at[
                twstrdate, "開盤價"
            ]
            # print('紅下影線=', loshadow)
        if loshadow > 0.03:
...
def upshadow(pf, twstrdate, historydataI):
    ...
    if pf.at[twstrdate, "開盤價"] > pf.at[twstrdate, "收盤價"]:
        upshadow = (pf.at[twstrdate, "最高價"] - pf.at[twstrdate, "開盤價"]) / pf.at[
            twstrdate, "開盤價"
        ]
    else:
        upshadow = (pf.at[twstrdate, "最高價"] - pf.at[twstrdate, "收盤價"]) / pf.at[
            twstrdate, "開盤價"
        ]
    if upshadow > 0.03:
...
def MAlowtouch(historydataI):
    ...
    try:
        if (
            historydataI.at[x - 3, "20MA"] < historydataI.at[x - 3, "收盤價"]
            and historydataI.at[x - 2, "20MA"] < historydataI.at[x - 2, "收盤價"]
            and historydataI.at[x - 1, "最低價"]
            < historydataI.at[x - 1, "20MA"]
            < historydataI.at[x - 1, "收盤價"]
        ):
            messeage = "/n", "20MA有支撐"
    except:
        pass
...
for i in range(0, len(fostocklist)):  # len(fostocklist)
    ...
    # 峰值告警
    twomes = alert(stock_no, stock_name, periodhigh, todayprice, yesprice, periodlow)
    # 融資告警
    ...
    comMes = (
        twomes + upShadowAlert + messeagMG + kdmesseage
    )  # + Finmessage2+ MAsupmes+ loShadowAlert
